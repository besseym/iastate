...
function create_table($db_link, $database, $table_name, $table_sql){
	...
	if( !$check ) {
		...
		if($create_table){
			error_log("DB INSTALL: Table created.");
		}
		else {
			error_log("DB INSTALL: Table creation failed.");
		}
	}
	else {
		error_log("DB INSTALL: Table already exists.");
	}
}
